...
class Book(db.Model):
    ...
    genres : Mapped[list['Genre']] = db.relationship(back_populates='books', uselist=True,  secondary='book_genre')
    reviews : Mapped[list['Review']] = db.relationship(back_populates='book', uselist=True, cascade='all, delete-orphan')

class Genre(db.Model):
    id : Mapped[int] = mapped_column(primary_key=True)
    genre_name : Mapped[str] = mapped_column(db.String(200), nullable=False, unique=True)

    books : Mapped[list['Book']] = db.relationship(back_populates='genres', uselist=True, secondary='book_genre', )

class book_genre(db.Model):
    book_id = mapped_column(db.ForeignKey('book.id'), primary_key=True)
    genre_id = mapped_column(db.ForeignKey('genre.id'), primary_key=True)
